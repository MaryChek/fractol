...
__kernel void	check_iter(__global int *img, int color, int win_wid, int win_hig,
					int iter_max, double zoom, int y_shift, int x_shift, t_coord mouse, t_coord consts, double jul_fr)
{
	int			iter;
	double		tmp;
	double		col;
	...
	iter = -1;
	while ((x_st * x_st + y_st * y_st) <= 200.00 && ++iter < iter_max)
	{
		tmp = x_st;
		x_st = tmp * tmp - y_st * y_st + consts.x;
		y_st = 2 * tmp * y_st + consts.y;
	}
